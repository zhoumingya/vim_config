...
function! s:GetDirectorySearchList()
	...
	let common_dirs = [ root ]
	let extra_dir = g:EasyGrepExtraRoot
	for d in extra_dir
		call add(common_dirs, s:FileEscape(d))
	endfor
...
" Selection Functions {{{
" GrepSelectionEx {{{
function! <sid>GrepSelectionEx(extra_root_id, add, wholeword)
	if a:extra_root_id == 1
		let g:EasyGrepExtraRoot = g:EasyGrepExtraRoot1
	else
		let g:EasyGrepExtraRoot = g:EasyGrepExtraRoot2
	endif
    call s:SaveVariable("g:EasyGrepExtraRoot")
	call <sid>GrepSelection(a:add, a:wholeword)
endfunction
...
" }}}
" GrepCurrentWordEx {{{
function! <sid>GrepCurrentWordEx(extra_root_id, add, wholeword)
	if a:extra_root_id == 1
		let g:EasyGrepExtraRoot = g:EasyGrepExtraRoot1
	else
		let g:EasyGrepExtraRoot = g:EasyGrepExtraRoot2
    ...
    call s:SaveVariable("g:EasyGrepExtraRoot")
	call <sid>GrepCurrentWord(a:add, a:wholeword)
endfunction
...
" GrepCommandLine {{{
function! s:GrepCommandLine(extra_root_id, argv, add)
	if a:extra_root_id == 1
		let g:EasyGrepExtraRoot = g:EasyGrepExtraRoot1
	else
		let g:EasyGrepExtraRoot = g:EasyGrepExtraRoot2
...
" Commands {{{
command! -nargs=+ Grep :call s:GrepCommandLine(1, <q-args> , "")
command! -nargs=+ Grepf :call s:GrepCommandLine(2, <q-args> , "")
command! -nargs=+ GrepAdd :call s:GrepCommandLine(1, <q-args>, "add")
...
if !hasmapto("<plug>EgMapGrepCurrentWord1_v")
    map <silent> <Leader>vv <plug>EgMapGrepCurrentWord1_v
endif
if !hasmapto("<plug>EgMapGrepCurrentWord2_v")
    map <silent> <Leader>vf <plug>EgMapGrepCurrentWord2_v
endif
if !hasmapto("<plug>EgMapGrepSelection1_v")
    vmap <silent> <Leader>vv <plug>EgMapGrepSelection1_v
endif
if !hasmapto("<plug>EgMapGrepSelection2_v")
    vmap <silent> <Leader>vf <plug>EgMapGrepSelection2_v
endif
...
if !exists("g:EasyGrepMappingsSet")
    ...
    nmap <silent> <unique> <script> <plug>EgMapGrepCurrentWord1_v    :call <sid>GrepCurrentWordEx(1,"", 0)<CR>
    nmap <silent> <unique> <script> <plug>EgMapGrepCurrentWord2_v    :call <sid>GrepCurrentWordEx(2,"", 0)<CR>
    vmap <silent> <unique> <script> <plug>EgMapGrepSelection1_v     y:call <sid>GrepSelectionEx(1,"", 0)<CR>
    vmap <silent> <unique> <script> <plug>EgMapGrepSelection2_v     y:call <sid>GrepSelectionEx(2,"", 0)<CR>
...
if !exists("g:EasyGrepExtraRoot1")
	let g:EasyGrepExtraRoot1 = []
endif

if !exists("g:EasyGrepExtraRoot2")
